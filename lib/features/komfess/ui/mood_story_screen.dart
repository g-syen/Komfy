...
class _MoodStoryScreenState extends State<MoodStoryScreen> {
  ...
  Future<void> _saveMoodToFirebase() async {
    ...
    final originalMoodData = moodEntry.toMap();

    // 2. Prepare the map to be sent to Firestore
    final Map<String, dynamic> encryptedDataForFirestore = {
      // Add the unencrypted fields first
      'userId': userId,
    ...
    try {
      ...
      if(docSnap.exists) {
        final String? publicKeyString = docSnap.data()?['publicKey'];
        final RSAPublicKey? publicKey = deserializePublicKeyFromJsonBase64(publicKeyString!);
        for (var entry in originalMoodData.entries) {
          final key = entry.key;
          final value = entry.value;
          String stringToEncrypt;

          // Skip the fields we're not encrypting
          if (key == 'userId' || key == 'timestamp') {
            continue;
          }

          // Convert lists and other non-string types to a JSON String
          if (value is List) {
            stringToEncrypt = jsonEncode(value);
          } else {
            stringToEncrypt = value.toString();
          }

          // Encrypt the string representation of the value
          encryptedDataForFirestore[key] = rsaEncryptSingleBlock(stringToEncrypt, publicKey!);
        }
      }


      // 4. Save the fully encrypted map to Firestore
      await FirebaseFirestore.instance.collection('mood_entries').add(encryptedDataForFirestore);
